...
class PrepTimeFilter:
    def __init__(self):
        self.SGTree = SGTree()
        self.initTree()

    def initTree(self):
        for num in range(5, 91, 5): #from 5 to 90, with increments of 5
            self.SGTree.add(num, -1) #add -1 as temp values

        for index in range(len(recipes_data)):
            self.SGTree.add(recipes_data[index]['prep_time'], index)

    def filter(self, filterVal):
        print(f"FILTERVAL: {filterVal}")
        indexlist = self.SGTree.findAndFilter(float(filterVal))
        obtainedNameList = []
        for index in indexlist:
            obtainedNameList.append(recipes_data[index]['preprocessedname'])
            
        return obtainedNameList
