...
# Load JSON data
with open(r'D:\SCHOOL\1STSEM_2NDYR\CMSC123\FINALPROJECT\project_root\assets\recipesAll.json', 'r') as file:
...
for index in indices_list:
    print(f"NAME: {recipes_data[index]['name']} - TIME: {recipes_data[index]['prep_time']}")
#````````````````````````````````````````````